...
def main():
    ...
    # ── FC 연결 및 준비 ───────────────────────────────────────────────────────
    # start_vision=True: EKF wait 동안 UWB VPE를 먼저 전송 → pos_rel 확보
    # connect() 반환 후 vision_pause=True로 lib_common VPE를 중단하고
    # 아래 _vision_loop(Tag+UWB 융합)로 교체
    c, stop, cache, lock = connect(uwb, start_vision=True)
    if c is None:
        return

    # ── VPE 루프 교체 (이륙 전) ──────────────────────────────────────────────
    # lib_common의 단순 UWB VPE를 멈추고 Tag+UWB 융합 루프로 교체
    with lock:
