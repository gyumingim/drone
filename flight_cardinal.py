...
def _flight(conn, uwb):
    # 북(+x) → 동(+y) → 남(-x) → 서(-y) 순서로 30cm씩 이동 후 착륙
    try:
        ...
        pos0 = conn.recv_match(type='LOCAL_POSITION_NED', blocking=True, timeout=2)
        sx = pos0.x if pos0 else 0.0
        sy = pos0.y if pos0 else 0.0
        flog(f"Start pos: x={sx:.2f} y={sy:.2f} z={hover_z:.2f}")

        # 웨이포인트: (label, dx, dy)  — NED 기준 (북=+x, 동=+y)
        waypoints = [
            ("북 +30cm", sx + STEP_M, sy),
        ...
        for label, tx, ty in waypoints:
            ...
            if not goto(conn, tx, ty, hover_z):
                flog("[MISSION] goto timeout — aborting, landing")
                land(conn)
                return
            flog(f"  {label} 도착 — {STEP_HOLD}s 정지")
            if not hold(conn, tx, ty, hover_z, STEP_HOLD, uwb=uwb):
                flog("[MISSION] UWB lost during hold — aborting")
                return
